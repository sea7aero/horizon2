...
void APRS::set_position(float latitude, float longitude, float altitude_feet) {
  ...
  // In theory, one should reduce the path when at altitude, but there seems to be consensus that
  // WIDE1-1, WIDE2-1 is a good compromise for balloons to use.
  message_.set_ttls(1, 1);
}

void APRS::set_comment(const char *format, ...) {
  va_list args;
  va_start(args, format);
  vsnprintf(message_.comment, MAX_COMMENT_LEN, format, args);
  va_end(args);
}
