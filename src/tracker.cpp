...
double prevAltitude_feet = 0;
unsigned long prevAltitudeMillis = 0;
EMAFilter ascentRateFilter(0.01);
...
void loop() {
  ...
  if(gps.location.isValid()) {
    ...
    aprs.set_position(gps.location.lat(), gps.location.lng(), altitude_feet);
    if(prevAltitudeMillis > 0) {
        unsigned long duration = (currentMillis - prevAltitudeMillis) / 1000;
        double ascent = altitude_feet - prevAltitude_feet;
        int ascentRate = (int)ascentRateFilter.add(ascent / duration);
    
        aprs.set_comment(PRIVATE_COMMENT, ascentRate);
    }

    prevAltitude_feet = altitude_feet;
    prevAltitudeMillis = currentMillis;
  }
  ...
  if(currentMillis > nextTelemetry) {
    ...
    nextTelemetry = currentMillis + 1000;
